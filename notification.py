...
class Notification:
	...
	def from_group(group, identifier):
		...
		actions = {name.lower().replace("’","'"): action for action, name in actions.items()}
...
class NotificationMonitor(object):
	...
	def perform_action(self, action: str, index: Optional[int] = None, app_name: str = None):
		self.update_notifications()
		...
		if index is not None:
			...
			try:
				...
				app.notify(f'Unable to locate notification for {app_name}', 'Try again?')
				return False
	...
	def update_notifications(self, adding=[]):
		...
		# XXX create_spoken_forms_from_list doesn't handle apostrophes correctly
		# https://github.com/knausj85/knausj_talon/issues/780
		apostrophe_words = {word.replace("'", " "): word for word in
							chain.from_iterable(action.split() for action in notification_actions)
							if "'" in word}
